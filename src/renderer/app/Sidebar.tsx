    ...
    >
      ...
      >
        <div className="p-4 space-y-6 overflow-y-auto no-scrollbar">
          <div>
            <p className="label-caps px-3 mb-2">{t('sidebar.workspace')}</p>
        ...
        <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
          <div className="flex items-center justify-between px-4 py-3">
            <p className="label-caps">{t('sidebar.courses')}</p>
            <p className="text-xs text-[var(--color-muted)]">
              {loadingCourses
                ? t('common.loading')
                : t('sidebar.total', { count: courses.length })}
            </p>
          </div>
            ...
            {courses.length > 0 ? (
                  ...
                  <div key={course.id}>
                    <div className="flex items-start gap-1">
                      ...
                      {renamingCourseId === course.id ? (
                        ...
                        <button
                          aria-label={t('sidebar.openCourse', {
                            name: course.name,
                          })}
                        ...
                        >
                          ...
                          <span className="mt-0.5 block text-xs text-[var(--color-muted)]">
                            {getCourseMetaLine(course).join(' · ') ||
                              t('sidebar.courseMetaFallback')}
                          </span>
                        </button>
                      )}
                    </div>
                    ...
                    {isExpanded && (
                      <div className="ml-8 mt-1 space-y-0.5 border-l border-[var(--color-line)] pl-3">
                        {isLoadingPapers ? (
                          ...
                          papers.map((paper) => (
                            <button
                              aria-label={t('sidebar.openPaper', {
                                title: paper.title,
                              })}
              ...
              <div className="px-4 py-4 text-sm leading-6 text-[var(--color-muted)]">
                {loadingCourses
                  ...
                  : (emptyCoursesMessage ?? t('sidebar.emptyCoursesDefault'))}
              </div>
            )}
          </div>
        </div>
