...
const formatJournalArticle = (fields: Fields): ReferenceFormattedSegment[] => {
  ...
  segs.push({
    text: formatAuthors(authors(fields)) + yearSegment(fields),
    italic: false,
  });
...
const formatEditedBookChapter = (
  ...
  const editors = (
    Array.isArray(fields.editors) ? fields.editors : []
  ) as ReferenceAuthor[];
  if (editors.length > 0) {
...
const formatWebsite = (fields: Fields): ReferenceFormattedSegment[] => {
  ...
  if (retrievalDate && url) {
    segs.push({
      text: `Retrieved ${retrievalDate}, from ${url}`,
      italic: false,
    });
  } else if (url) {
...
const formatConferencePaper = (fields: Fields): ReferenceFormattedSegment[] => {
  ...
  if (conference) {
    ...
    segs.push({
      text: ` [Paper presentation]. ${conference}${locationSuffix}.`,
      italic: false,
    });
  } else {
    segs.push({ text: '.', italic: false });
...
export const formatInTextCitation = (entry: ReferenceEntry): string => {
  const a = (
    Array.isArray(entry.fields.authors) ? entry.fields.authors : []
  ) as ReferenceAuthor[];
  const year =
    typeof entry.fields.year === 'string' && entry.fields.year.trim()
